...
POST /orders/{order_id}/easypost/rates  — create shipment, return USPS rates
...
async def get_rates(order_id: int, body: RatesRequest, db: AsyncSession = Depends(get_db)):
    """Create an EasyPost shipment and return available USPS rates."""
    ...
    usps_rates = filter_usps_rates(all_rates) or all_rates  # fallback to all if no USPS
